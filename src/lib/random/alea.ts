 ...
 */
export const mkAlea = (seed: string = defaultSeed()): AleaPRNG =>
  restoreAlea(mkState(seed));
 ...
 */
export const fract53 = (state: AleaState): number =>
  random(state) + ((random(state) * 0x200000) | 0) * 1.1102230246251565e-16; // 2^-53
 ...
 */
export const nextT = <T>(f: (n: number) => T) => (state: AleaState): T =>
  f(random(state));
